using Cysharp.Threading.Tasks;
using System.Threading;
using UnityEngine;
using VContainer;

public class JumpModuleAbility : AbilityBase
{
    [SerializeField] private float _jumpPower = 8f;
    [SerializeField] private float _cooldownTime = 2f;

    private PlayerContext _playerContext;
    private CancellationTokenSource _cooldownCts;
    private bool _isCooldown;

    [Inject]
    public void Construct(PlayerContext playerContext)
    {
        _playerContext = playerContext;
        SubscribeToInput();
    }

    private void SubscribeToInput() => _playerContext.Input.JumpStarted += Jump;
    private void UnsubscribeFromInput() => _playerContext.Input.JumpStarted -= Jump;

    private void Jump()
    {
        if (!IsActive)
            return;

        if (_isCooldown)
            return;

        _playerContext.Movement.Jump(_jumpPower);

        _cooldownCts = new CancellationTokenSource();
        StartAbilityCooldown(_cooldownCts.Token).Forget();
    }

    private async UniTask StartAbilityCooldown(CancellationToken cts)
    {
        _isCooldown = true;
        await UniTask.WaitForSeconds(_cooldownTime, cancellationToken: cts);
        _isCooldown = false;
    }

    private void OnDestroy()
    {
        UnsubscribeFromInput();
        _cooldownCts?.Cancel();
        _cooldownCts?.Dispose();
    }
}
